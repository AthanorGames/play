...
The set here was generated through `/asset-engine` (prompts in
`prompts/ground/`) and made tileable by `tool/make_seamless.py`, which
also reports whether it worked:

```sh
cd /asset-engine && scripts/run_workflow.py workflows/api/txt2img_qwen.json \
...
/asset-engine/scripts/make_seamless.py \
    /asset-engine/output/ground/plains_00001_.png \
